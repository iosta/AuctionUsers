...
public class MenuUsrPrincipal implements Menu {
    ...
    private void mostrarPujasUsuario() {
        List<PujaItem> pujasArts = AuctionService.obtenerPujasVigentesUsuario(username);
    ...
    private void ofertarItem() {
        ...
        boolean esUsuario = AuctionService.autenticar(nombreUsuario, passwordUsuario) && !AuctionService.esAdmin(nombreUsuario);
        if (!esUsuario) {
        ...
        if (b) {
            b = AuctionService.ofrecerArticulo(item);
        }
